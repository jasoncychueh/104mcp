...
2. 在專案根目錄的 `.mcp.json` 加入：
   ...
   `MCP104_ACCOUNT` 是必填的，填你登入 104 用的 email；不同帳號的資料（登入憑證、
   候選人狀態、每日發送計數）會各自放在自己的資料夾。`timeout` 是單一工具呼叫的上限（毫秒），120 秒是本專案建議
   的值，請照抄。
...
- **換帳號**：每個 `MCP104_ACCOUNT` 各自有一個資料夾（登入憑證、候選人狀態、發送計數
  都分開），直接改這個值就好，不用另外設定。第二個帳號第一次用時要再 `login()` 一次。
...
| `MCP104_DATA_DIR` | 選填，預設是使用者目錄下的 `mcp104/<帳號>` | 資料庫、登入憑證、節流狀態都存在這裡。設了就原樣使用、不再依帳號分子目錄 |
