...
| 工具 | 給你什麼 |
|---|---|
| `list_recommended_resumes` | 104 演算法推薦，量最大 |
| `list_matched_resumes` | 條件配對，通常較精準 |
| `search_resumes` | 關鍵字搜尋，唯一會回 `master_url` 的路由 |

這三個都**不扣履歷瀏覽配額**（repo 的 `docs/104-site-facts.md` §6b.4 有對照實驗）。要全部
就翻完所有頁，不要只抓第一頁然後說「今天的」——使用者說「看推薦履歷」多半是指整池，不是
指今天新增的那幾筆。不確定就問。

把清單存成一個 JSON 檔（`cands.json`）再開始篩，不要留在 context 裡。幾百筆清單塞進對話會
把後面讀履歷的空間吃光。
